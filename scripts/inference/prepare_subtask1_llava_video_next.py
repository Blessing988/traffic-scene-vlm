...
MODEL_DIR = "saves/llava_next_video/lora/sft"
# 1 load processor & model
processor = LlavaNextVideoProcessor.from_pretrained(MODEL_DIR)
model = LlavaNextVideoForConditionalGeneration.from_pretrained(
    MODEL_DIR,
    torch_dtype=torch.float16,          # or bfloat16 / float32
...
def optimized_inference(frames, prompt):
    ...
    # 2  conversation in *string* form with a <video> token
    conversation = [
        {
            "role": "user",
            "content": f"<video>\n{prompt}"
    ...
    inputs = processor(text=prompt,
                    videos=frames,          # numpy array (T, H, W, 3)
                    padding=True,
                    return_tensors="pt").to(model.device)

    out = model.generate(**inputs, max_new_tokens=1024)
    res = processor.decode(out[0][inputs["input_ids"].shape[-1]:],
...
def save_results():
    ...
    overhead_file = Path(f"test_data_predictions_subtask1_overhead_views_llava_next_video_{timestamp}_10_epochs.json")
    normal_file = Path(f"test_data_predictions_subtask1_normal_views_llava_next_video_{timestamp}_10_epochs.json")
    external_file = Path(f"test_data_predictions_subtask1_external_views_llava_next_video_{timestamp}_10_epochs.json")
    ...
    if combined_json:
        combined_file = Path(f"test_data_predictions_subtask1_ALL_VIEWS_llava_next_video_{timestamp}_10_epochs.json")
